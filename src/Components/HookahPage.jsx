...
function HookahsPage() {
  const MySwal = withReactContent(Swal);
  const [getHookahs, setGetHookahs] = useState([]);
  const [modalIsOpen, setModalIsOpen] = useState(false);
  const [isLoading, setIsLoading] = useState(true);
  const fetchHookahs = () => {
    setIsLoading(true);
    console.log(isLoading);
    fetch("https://kale-cafe.com/api/getHookahs", {
      method: "GET",
      headers: {
  ...
  const deleteHookah = (_id) => {
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        fetch(`https://kale-cafe.com/api/deleteHookah/${_id}`, {
          method: "DELETE",
        })
          .then((res) => res.json())
          .then((data) => {
            console.log(data);
            setGetHookahs(getHookahs.filter((Hookah) => Hookah._id !== _id));
            MySwal.fire({
              title: "Deleted!",
  ...
  const [editHookahId, setEditHookahId] = useState(null);
  ...
  // Open the modal for adding a new Hookah item
  const openAddModal = () => {
    resetForm();
    setIsEditMode(false);
    setModalIsOpen(true);
  ...
  // Open the modal for editing an existing Hookah item
  const openEditModal = (Hookah) => {
    console.log(Hookah);
    setFormData({
      name: Hookah.name,
  ...
  const handleFormSubmit = (e) => {
    ...
    const url = isEditMode
      ? `https://kale-cafe.com/api/updateHookah/${editHookahId}`
      : "https://kale-cafe.com/api/addHookah";
    ...
    fetch(url, {
      ...
      .then((data) => {
        ...
        closeModal();
        fetchHookahs(); // Fetch updated Hookah list after adding/updating
        setFeedbackMessage(
          isEditMode
            ? "Hookah updated successfully!"
            : "Hookah added successfully!"
        );
      })
      .catch((error) => {
        setIsSubmitting(false);
  ...
  const onItemSearch = () => {};
  const Hookah = getHookahs.map((Hookah, index) => (
    <tr>
      {/* <script>{Hookah}</script> */}
      {/* className="border rounded d-flex flex-row-reverse gap-2 p-2" */}
      <td className="categorys-info">{Hookah.name}</td>
      <td className="categorys-info">{Hookah.category}</td>
      <td className="categorys-info">{Hookah.price}</td>
      <td className="d-flex flex-row-reverse align-items-center gap-2 justify-content-start">
        <div
          ...
          onClick={() => deleteHookah(Hookah._id)}
        >
          <svg
    ...
    <>
      ...
      <>
        <div className="d-flex justify-content-center pt-3">
          <h1 className="text-decoration-underline">الأراكيل</h1>
        </div>
        ...
        <Modal
          ...
          contentLabel={isEditMode ? "Edit Hookah" : "Add Hookah"}
          className="modal"
          overlayClassName="overlay"
        >
          <h2 className="d-flex justify-content-center mb-2 pb-2">
            {isEditMode ? "Edit Hookah" : "Add Hookah"}
          </h2>
