...
function DessertsPage() {
  ...
  const [getDesserts, setGetDesserts] = useState([]);
  const [modalIsOpen, setModalIsOpen] = useState(false);
  const [isLoading, setIsLoading] = useState(true);
  const fetchDesserts = () => {
    setIsLoading(true);
    console.log(isLoading);
    fetch("https://kale-cafe-server.onrender.com/api/getDesserts", {
      method: "GET",
      headers: {
  ...
  const deleteDessert = (_id) => {
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        fetch(
          `https://kale-cafe-server.onrender.com/api/deleteDessert/${_id}`,
          {
            method: "DELETE",
          }
        )
          ...
          .then((data) => {
            console.log(data);
            setGetDesserts(
              getDesserts.filter((Dessert) => Dessert._id !== _id)
            );
            MySwal.fire({
              title: "Deleted!",
  ...
  // Open the modal for adding a new Dessert item
  const openAddModal = () => {
    resetForm();
    setIsEditMode(false);
    setModalIsOpen(true);
  ...
  // Open the modal for editing an existing Dessert item
  const openEditModal = (Dessert) => {
    console.log(Dessert);
    setFormData({
      name: Dessert.name,
  ...
  const handleFormSubmit = (e) => {
    ...
    const url = isEditMode
      ? `https://kale-cafe-server.onrender.com/api/updateDessert/${editDessertId}`
      : "https://kale-cafe-server.onrender.com/api/addDessert";
    ...
    fetch(url, {
      ...
      .then((data) => {
        ...
        closeModal();
        fetchDesserts(); // Fetch updated Dessert list after adding/updating
        setFeedbackMessage(
          isEditMode
  ...
  const onItemSearch = () => {};
  const Dessert = getDesserts.map((Dessert, index) => (
    <tr>
      {/* <script>{Dessert}</script> */}
      {/* className="border rounded d-flex flex-row-reverse gap-2 p-2" */}
      <td className="categorys-info">{Dessert.name}</td>
      <td className="categorys-info">{Dessert.category}</td>
      <td className="categorys-info">{Dessert.price}</td>
    ...
    <>
      ...
      <>
        <div className="d-flex justify-content-center pt-3">
          <h1 className="text-decoration-underline">الحلويات</h1>
        </div>
        ...
        <Modal
          ...
          contentLabel={isEditMode ? "Edit Dessert" : "Add Dessert"}
          className="modal"
          overlayClassName="overlay"
        >
          <h2 className="d-flex justify-content-center mb-2 pb-2">
            {isEditMode ? "Edit Dessert" : "Add Dessert"}
          </h2>
