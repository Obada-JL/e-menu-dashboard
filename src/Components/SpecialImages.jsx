...
function SpecialImagesPage() {
  ...
  const fetchImages = async () => {
    try {
      ...
      const response = await fetch(
        "https://kale-cafe-server.onrender.com/api/getSpecialImages",
        {
          method: "GET",
          headers: {
  ...
  const deleteImage = (_id) => {
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        fetch(
          `https://kale-cafe-server.onrender.com/api/deleteSpecialImage/${_id}`,
          {
            method: "DELETE",
          }
        )
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      const url = "https://kale-cafe-server.onrender.com/api/addSpecialImage";
  ...
  return (
    <>
      <div className="d-flex justify-content-center pt-3">
        <h1 className="text-decoration-underline">الصور الرئيسية</h1>
      </div>
